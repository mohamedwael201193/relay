...
async function probeOne(
  ...
  opts: { requireAsks: boolean; skip: Set<string> },
...
export async function discoverLiveMarket(opts: {
  ...
  while (Date.now() <= deadline) {
    ...
    const filtered = live.filter((m) => intervalMatches(m.intervalSec, opts.intervalSec));
    const pool = filtered.length > 0 ? filtered : opts.intervalSec ? [] : live;
    const found: LivePick[] = [];
    for (const m of pool) {
      try {
        const pick = await probeOne(exchange, client, m, { requireAsks, skip });
        if (!pick) continue;
        if (opts.maxExpiryHorizonSec != null) {
          const remainingSec = Number((pick.expireNs - nowNs()) / 1_000_000_000n);
          if (remainingSec > opts.maxExpiryHorizonSec) continue;
        }
        found.push(pick);
      } catch {
        /* skip broken row */
